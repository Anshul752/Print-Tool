...
import fitz,json,os,sys,threading,requests,shutil,subprocess,time,zipfile,random,cv2,winshell
...
Print_Data_Folder = os.path.join(os.getcwd(), "Print Data")
...
def load_buttons():
    default_buttons = [
        ...
        ("Card-1", "#28A745"),        # Green
        ("Card-2", "#DC3545"),        # Red
        ("Card-3", "#6F42C1"),        # Purple
        ("Card-4", "#FD7E14"),        # Orange
        ("Card-5", "#20C997"),        # Teal
    ]
...
def process_selected_file(file_selected):
...
def upload_doc():
    file_selected = filedialog.askopenfilename(filetypes=[("Files", "*.png;*.jpg;*.jpeg;*.gif;*.pdf")])
    process_selected_file(file_selected)

# Drag & drop handler
def on_drop(event):
    file_selected = event.data
    # Handle {braces} (Windows quirk for multiple files)
    if file_selected.startswith('{') and file_selected.endswith('}'):
        file_selected = file_selected[1:-1]
    process_selected_file(file_selected)
...
def update_preview():
    """ Updates the canvas with the zoomed image or PDF pages. """
    ...
    if file_path.endswith(".pdf"):
        ...
        try:
            ...
            for i in range(len(doc)):  # Show pages 1 to nth
...
def open_settings():
    ...
    notebook.add(tab4,text="License Mangement")
    ...
    def create_license_tab(tab4):
        ...
        status_label = ttk.Label(tab4, text="License Status: Checking...", font=STATUS_FONT, foreground="#003366")
        status_label.pack(pady=10)
        # --------- PC ID Label ---------
        pc_id_label = ttk.Label(tab4, text=f"Your PC ID: {license_manager.get_pc_identifier()}", font=LABEL_FONT, foreground="#333333")
        pc_id_label.pack(pady=5)
        # --------- Entry Label ---------
        entry_label = ttk.Label(tab4, text="Enter License Key:", font=LABEL_FONT, foreground="#003366")
        entry_label.pack(pady=(20, 5))
        # --------- Entry Box ---------
        license_entry = ttk.Entry(tab4, width=40, font=ENTRY_FONT)
        license_entry.pack(pady=5)
        # --------- Activate Button ---------
        style = ttk.Style()
        style.configure("Blue.TButton", font=BUTTON_FONT, foreground='Red', background='#007BFF')
        ...
        def activate_key():
            ...
            if not license_key:
                ...
                return
            success = license_manager.activate_license(license_key)
            if success:
                messagebox.showinfo("Success", "License activated successfully!")
                refresh_status()
            else:
                messagebox.showerror("Invalid", "Invalid or already used license key.")
        activate_button = ttk.Button(tab4, text="Activate License", command=activate_key, style="Blue.TButton")
        activate_button.pack(pady=10)
        # --------- Refresh Status ---------
        def refresh_status():
            status = license_manager.is_license_valid()
            if status == "licensed":
                status_label.config(text="License Status: ✅ Activated", foreground='green')
                # Hide Entry Label, Entry Box, Activate Button
                entry_label.pack_forget()
                license_entry.pack_forget()
                activate_button.pack_forget()
            else:
                # Show Entry Label, Entry Box, Activate Button
                entry_label.pack(pady=(20, 5))
                license_entry.pack(pady=5)
                activate_button.pack(pady=10)
                if status and str(status).startswith("trial_"):
                    try:
                        days_left = int(status.split("_")[1])
                        status_label.config(text=f"License Status: 🕒 Trial Active ({days_left} days left)", foreground='orange')
                    except:
                        status_label.config(text="License Status: 🕒 Trial Active", foreground='orange')
                elif status == "expired":
                    status_label.config(text="License Status: ❌ Trial Expired", foreground='red')
                elif status == "blocked":
                    status_label.config(text="License Status: ❌ Blocked (System date rollback detected)", foreground='red')
                else:
                    status_label.config(text="License Status: ❌ Not Activated", foreground='red')
        refresh_status()
    create_license_tab(tab4)
...
def create_desktop_shortcut():
    if True:
        try:
            ...
            shortcut_path = os.path.join(desktop, "ID Print Tool.lnk")
            exe_path = sys.executable  # Path to your running .exe
            exe_dir = os.path.dirname(exe_path)
            icon_path = os.path.join(exe_dir, "id_card_icon.ico")  # Optional custom icon
            if not os.path.exists(icon_path):
                icon_path = exe_path  # fallback to exe icon if ico not found
            shell = Dispatch('WScript.Shell')
            shortcut = shell.CreateShortCut(shortcut_path)
            shortcut.TargetPath = exe_path
            shortcut.WorkingDirectory = exe_dir
            shortcut.IconLocation = icon_path
            shortcut.Description = "Launch ID Print Tool"
            shortcut.save()
            print("[INFO] Desktop shortcut created successfully.")
        except Exception as e:
...
# Attach drag and drop support to the frame
frame.drop_target_register(DND_FILES)
frame.dnd_bind('<<Drop>>', on_drop)

# Upload Document Button
upload_btn = tk.Button(frame, text="🖼 Drag or Upload File", command=upload_doc, font=("Arial", 12, "bold"), bg="#007BFF", fg="white")
...
paper_dropdown["values"] = ["4x6 Paper","PVC Card","A4 Paper","A4 Paper U+D","A4 Paper L+R"]#"A4 Paper Full"
...
# 1.12 New_multi_ID_card_Print_on_A4_sheet+edit_image 
# 1.13 PVC Card Print One by One Print Open EPson Photo+
# 1.14 Preview window close Fix, Password without entry submit fix,desktop shortcut
# 1.15_license_manager_system_added_with_3_days_trial,toggle and reverse fixed in All,drag upload
# 1.16 license_30_days_admin_portal
