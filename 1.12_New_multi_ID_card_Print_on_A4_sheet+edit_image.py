...
def show_print_preview():
    try:
        ...
        # elif paper_size == "A4 Paper Full":
        #     canvas_width, canvas_height = 2480, 3508 
        #     img_width, img_height = 2356, 3332
        #     front_pos = (70, 70)  
        #     back_pos = None  
        #     add_border = False
...
def print_id_card():
    try:
        ...
        if selected_document_type.get():  # For document types, save to Print Data
            save_dir = f"Print Data/{selected_document_type.get()}"
        else:  # For final prints, save to Print and Crop folder
...
def open_image_viewer():
    ...
    def show_image_view():
        ...
        tk.Button(button_frame, text="Next ➡", font=("Arial", 12, "bold"), bg="#007BFF",fg="black", command=next_image).grid(row=0, column=3, padx=5, pady=10, sticky="ew")

        tk.Button(button_frame, text="🔙 Back", font=("Arial", 12, "bold"), bg="orange",fg="black", command=show_folder_list).grid(row=0, column=4, padx=5, pady=10, sticky="ew")
...
def upload_document_for_crop(card_side):
    is_crop_and_print = True
    global file_path, doc, attempts, password_window, crop_card_side
...
def show_crop_preview(original_image, card_side):
    ...
    def start_crop():
        messagebox.showinfo("Crop start", "Please mark 4 points for croping .")
        points.clear()
        canvas.delete("all")
        update_canvas_image(get_transformed_image())
    ...
    def apply_perspective_crop():
        ...
        if card_side == "front":
            ...
            root.after(1000, lambda: upload_document_for_crop("back"))
        else:
...
paper_dropdown["values"] = ["4x6 Paper", "A4 Paper","A4 Paper U+D","A4 Paper L+R"]#"A4 Paper Full"
...
# image Viewer updated with Print Option , fixed image resize to boxes, folder name now image number too + open image folder location
